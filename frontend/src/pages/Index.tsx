...
const Index: React.FC = () => {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (bestSellersResponse.success) {
          console.log("Best sellers from API:", bestSellersResponse.data.length);
          // Transform API products to match local Product type
          const transformedBestSellers = bestSellersResponse.data.map((apiProduct: ApiProduct) => ({
            id: apiProduct.id,
            name: apiProduct.name,
            image: apiProduct.image,
            images: apiProduct.images?.map(img => img.url) || [apiProduct.image],
        ...
        if (expertsResponse.success) {
          ...
          // Transform API products to match local Product type
          const transformedExperts = expertsResponse.data.map((apiProduct: ApiProduct) => ({
            id: apiProduct.id,
            name: apiProduct.name,
            image: apiProduct.image,
            images: apiProduct.images?.map(img => img.url) || [apiProduct.image],
        ...
        const allApiProducts = [...(bestSellersResponse.data || []), ...(expertsResponse.data || [])];
        const transformedAllProducts = allApiProducts.map((apiProduct: ApiProduct) => ({
          id: apiProduct.id,
          name: apiProduct.name,
          image: apiProduct.image,
          images: apiProduct.images?.map(img => img.url) || [apiProduct.image],
        ...
        const bestSellerProducts = allProducts.filter((product: Product) => product.bestSeller).slice(0, 8);
        const expertProducts = allProducts.filter((product: Product) => product.lovedByExperts).slice(0, 4);
    ...
    return (
      <div className="bg-gradient-to-b from-[#fff9f2] to-[#f8fafc] overflow-x-hidden min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading Best Sellers & Expert Picks...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      {/* Hero Section */}
      <section className={`relative bg-gradient-to-b from-[#f8fafc] via-[#fff9f2] to-[#f8fafc] ${isDesktop ? 'min-h-[500px]' : ''}`} style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)' }}>
                    ...
                    {activeBanners.map((_, dotIndex) => (
                      <button
                        ...
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${
                          dotIndex === currentSlide ? "bg-[#F9A245] w-6" : "bg-white/50"
                        }`}
                      />
                    ))}
                  </div>
...
>
  <div className="w-full grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
    {[
      ...
      <div className="bg-gradient-to-b from-[#f8fafc] via-[#fff9f2] to-[#f8fafc] overflow-x-hidden" style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)' }}>
      ...
      >
        ...
        <div className="w-full relative z-10">
          <div className="text-center space-y-2 mb-8">
            <h2 className="font-bold text-gray-900 tracking-tight leading-tight text-[clamp(1.875rem,3vw,2.5rem)]">
              WHY CHOOSE <br className="sm:hidden" /><span className="text-[#F9A245]">O2 NUTRITION</span>
            </h2>
            <div className="mt-4">
              <div className="inline-flex items-center space-x-2 px-5 py-2.5 bg-gradient-to-r from-[#F9A245]/10 to-[#E86A12]/10 rounded-full border-2 border-[#F9A245]/30 backdrop-blur-sm relative overflow-hidden" style={{animation: 'softGlow 4s ease-in-out infinite', boxShadow: '0 0 10px rgba(249, 162, 69, 0.3)'}}>
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-[shimmer_4s_ease-in-out_infinite]" style={{animation: 'shimmer 4s ease-in-out infinite'}}></div>
    ...
    <div className="relative w-full h-[400px] xs:h-[450px] sm:h-[400px] md:h-[500px] lg:h-[600px] xl:h-[640px] overflow-hidden rounded-xl shadow-xl isolate">
      ...
      >
        ...
        <p className="mt-1.5 xs:mt-1.5 sm:mt-4 text-[clamp(0.75rem,2.8vw,1.2rem)] leading-tight xs:leading-tight sm:leading-relaxed opacity-90 font-light">
          Elevate your fitness journey with goal-based supplement bundles. Whether you're
          bulking, shredding, or staying fit — we've got your back.
        </p>
        ...
        <p className="mt-1 xs:mt-1 sm:mt-3 text-[clamp(0.75rem,2.8vw,1.2rem)] leading-tight xs:leading-tight sm:leading-relaxed opacity-90 font-light hidden xs:hidden sm:block">
          Shop by goal and discover tailored nutritional solutions trusted by athletes across
          the globe. Quality. Power. Results.
        </p>
      </div>
