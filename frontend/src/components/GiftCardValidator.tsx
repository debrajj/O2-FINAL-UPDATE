  ...
  const validateCode = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.API_BASE_URL}/api/gift-cards/validate`,
        {
          method: "POST",
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="space-y-4">
        ...
        {result && (
          <div
            className={`p-4 rounded-md ${result.success ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"} border`}
          >
            {result.success ? (
              <div>
